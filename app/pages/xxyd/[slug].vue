...
async function loadCategoryList() {
  try {
    ...
    if (categorys.value.length === 0) {
      categorys.value = [
        { title: '行业动态', value: 'cwgk' },
        { title: '规章制度', value: 'dwgk' },
      ]
    }
    ...
    // 失败时使用默认列表
    categorys.value = [
      { title: '行业动态', value: 'hydt' },
        { title: '规章制度', value: 'gzzd' },
    ]
    updateCurrentId()
  }
}
...
<template>
  <div class="ggtz-page">
    <!-- 页面标题 -->
    <Description text="学习园地" :image="image" />
