...
import { secendMenuById, headerArticlePageList } from '~/api'
...
// 监听路由query中的articleId参数
watch(
  () => route.query.articleId,
  (value) => {
    if (value) {
      articleId.value = value as string
      loadCategoryList()
    }
  },
  { immediate: true },
...
async function loadCategoryList() {
  ...
  try {
    const response = await secendMenuById({
...
function onCategoryChange(value: string) {
  router.push({
    path: '/ztjj',
    query: { articleId: articleId.value, id: value },
  })
}
</script>
...
<template>
  <div class="ztjj-page">
    <!-- 页面标题 -->
    <Description text="专题聚焦" :image="image" />
